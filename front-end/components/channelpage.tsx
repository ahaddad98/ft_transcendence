...
const ChannelPage = (props) => {
  ...
  // console.log(props.mychannels.conversation);
  
  // ${props.mychannels.conversation.Id}
  const fetchconsversation= async (id) => {
    console.log(id);
    const response = await axios.get(`http://localhost:3001/conversations/1/messages`, {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    });
    return response;
  };
  useEffect(() => {
      fetchconsversation(props.mychannels)
    }, [props.mychannels]);
  ...
  const sendmsg = async (e) => {
    ...
    await axios
    .post(
      `http://localhost:3001/messages/conversations/1/users/me`,
      {
        message: msg,
      },
      {
