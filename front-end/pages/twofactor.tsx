...
const TwoFactor = () => {
  ...
  const sendtoken = async (e) => {
    e.preventDefault();
    axios
      .post(`${process.env.NEXT_PUBLIC_FRONTEND_URL}:3001/login/twoFactor/`, {
        token: token,
        id: router.query.id,
      })
      .then((res) => {
      ...
      {router.query.id && (
        <div className="w-screen h-screen flex flex-col justify-center">
          <form className="flex  justify-center" onSubmit={sendtoken}>
            ...
            <input
              type="submit"
              className="px-8 rounded-r-lg bg-yellow-400  text-gray-800 font-bold p-4 uppercase border-orange-500 border-t border-b border-r"
            />
          </form>
        </div>
      )}
